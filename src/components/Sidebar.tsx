...
export const Sidebar: React.FC<SidebarProps> = ({ open = false, onClose }) => {
  ...
  return (
    <>
      ...
      />
      <div
        className={`fixed z-40 top-0 left-0 h-full transform transition-transform md:static md:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'} w-3/4 max-w-xs sm:w-64 bg-surface border-r border-border flex flex-col`}
      >
        <div className="flex items-center justify-between p-4 md:hidden">
          ...
          <button
            ...
            className="p-2 rounded-md bg-surface border border-border"
          >
            ✕
          </button>
        </div>
